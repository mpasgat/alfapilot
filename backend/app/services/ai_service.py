...
class OpenRouterService:
            ...
            try:
                ...
                if e.response.status_code in [401, 429, 404]:
                    if e.response.status_code == 429:
                        ...
                        error_msg = (
                            f"OpenRouter API authentication failed (invalid API key). "
                        )
                    ...
                    error_msg += (
                        "Try setting DEMO_MODE=true in .env for mock responses."
                    )
                    raise Exception(
                        f"OpenRouter API HTTP error: {e.response.status_code} - {error_msg}"
                    )
    ...
    def _get_demo_response(self, messages: List[Dict[str, str]]) -> str:
        ...
        if (
            "маркетинг" in user_message
            or "пост" in user_message
            or "marketing" in user_message
        ):
            return json.dumps(
                {
                    "post_variants": [
            ...
            "документ" in user_message
            or "письмо" in user_message
            or "document" in user_message
        ):
            return json.dumps(
                {
            ...
            "договор" in user_message
            or "контракт" in user_message
            or "legal" in user_message
            or "contract" in user_message
        ):
            return json.dumps(
                {
...
class AIService:
    ...
    async def generate_marketing_content(
        self, idea: str, tone: str, target_audience: str
    ) -> Dict[str, Any]:
        prompt = f"""
        ...
        messages = [
            {
                "role": "system",
                "content": "Ты эксперт по маркетингу и контент-стратегии. Отвечай только в формате JSON.",
            },
    ...
    async def generate_document(
        self, doc_type: str, content: str, style: str
    ) -> Dict[str, Any]:
        prompt = f"""
        ...
        messages = [
            {
                "role": "system",
                "content": "Ты профессиональный юрист и копирайтер. Отвечай только в формате JSON.",
            },
            {"role": "user", "content": prompt},
        ]
        ...
        try:
            ...
            return {
                ...
                "corrections": [
                    ...
                    "Уточните юридические термины",
                ],
                "suggestions": [
                    "Добавьте контактную информацию",
                    "Укажите сроки и даты",
                ],
            }

    async def analyze_contract(
        self, contract_text: str, analyze_risks: bool
    ) -> Dict[str, Any]:
        prompt = f"""
        ...
        messages = [
            {
                "role": "system",
                "content": "Ты опытный юрист с expertise в анализе договоров. Отвечай только в формате JSON.",
            },
            {"role": "user", "content": prompt},
        ]
        ...
        try:
            ...
            return {
                ...
                "risks": [
                    "Не указаны точные сроки выполнения",
                    "Неясные условия оплаты",
                    "Отсутствуют штрафные санкции",
                ],
                "recommendations": [
                    "Проконсультироваться с юристом",
                    "Уточнить условия расторжения",
                    "Добавить приложения с деталями",
                ],
                "todo_items": [
                    "Запросить дополнительные документы",
                    "Назначить встречу с юристом",
                    "Уточнить реквизиты сторон",
                ],
            }
    ...
    async def analyze_finance_data(
        ...
        messages = [
            {
                "role": "system",
                "content": "Ты финансовый аналитик с опытом в бизнес-аналитике. Отвечай только в формате JSON.",
            },
            {"role": "user", "content": prompt},
        ]
